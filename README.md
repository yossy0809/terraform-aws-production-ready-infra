...
| EC2 | Private Subnet (1a) に配置 | インターネットから直接到達不可 |
...
単一障害点（SPOF）を排除するため、ALB・RDSをMulti-AZ構成にしています。  
...
| WAF導入 | AWS WAFをALBに適用し、SQLインジェクション・XSS等を防御 |
| EC2 冗長化 | Auto Scaling Groupを使用したMulti-AZ構成・自動スケールアウト対応 |
