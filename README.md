...
![構成図](./diagram.png)
※ draw.ioで作成した構成図を diagram.png という名前でリポジトリに置いてください
...
| コンピューティング | Amazon EC2 (Auto Scaling / Multi-AZ) |
...
| Private Subnet | 10.0.3.0/24 (1a) / 10.0.4.0/24 (1c) | EC2 |
| DB Subnet | 10.0.5.0/24 (1a) / 10.0.6.0/24 (1c) | RDS |

### セキュリティ設計

セキュリティグループを連鎖参照（Chain）させることで、必要な通信のみを許可しています。
...
- 対象AWSアカウントにAdministratorAccess相当の権限
...
# 2. パスワードをtfvarsに設定（このファイルはGitに含まれません）
echo 'db_password = "任意のパスワード"' > terraform.tfvars
...
# 6. 確認後、リソース削除（検証環境の場合）
terraform destroy
```

### 注意事項

- `terraform.tfvars` は `.gitignore` で管理対象外にしています
- RDS Multi-AZ構成のため、applyに**10〜15分**程度かかります
- 検証後は必ず `terraform destroy` でリソースを削除してください（課金防止）

---

## 作者

- **インフラ経験**：5年（Linux / ネットワーク設計 / Windows Server / 仮想化 / 監視・障害対応）
